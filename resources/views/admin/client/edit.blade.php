			...
			<div class="d-flex flex-column justify-content-between h-100">
				...
				<div>
					<ul class="nav nav-tabs nav-pills db-custom-tabs gap-5 employee-tabs" id="myTab" role="tablist">
						...
						<li class="nav-item" role="presentation">
							<button class="nav-link" id="password-tab" data-bs-toggle="tab" data-bs-target="#password-T" type="button"
								role="tab" aria-controls="password-T" aria-selected="false">Password</button>
						</li>
					</ul>
				</div>
			</div>
							...
							<div class="row">
								<div class="col-12 mb-3">
									<div class="form-group">
										...
										<input type="password" class="form-control db-custom-input {{ $errors->has('password') ? ' is-invalid' : '' }}" name="password" id="password">
										@if ($errors->has('password'))
										<span class="text-danger">
										{{ $errors->first('password') }}
										</span>
								...
								<div class="col-12 mb-3">
									<div class="form-group">
										<label class="db-label" for="password_confirmation">Confirm Password</label>
										<input type="password" class="form-control db-custom-input {{ $errors->has('password_confirmation') ? ' is-invalid' : '' }}" name="password_confirmation" id="password_confirmation">
...
<script>
	$(document).ready(function () {
		...
		// Use Bootstrap's default tab functionality
		// No need for custom tab handling - Bootstrap handles it automatically
	...
	// Delete Administrator functionality
	$(document).on('click', '.delete-admin', function() {
		var adminId = $(this).data('admin-id');
		var adminName = $(this).data('admin-name');
		var companyId = '{{ $company->id }}'; // Get the company ID from the view
			...
			$.ajax({
				...
				success: function(response) {
					if (response.status) {
						...
						$('button[data-admin-id="' + adminId + '"]').closest('tr').remove();
				...
				error: function(xhr) {
					if (xhr.status === 403) {
						alert('Unauthorized to delete this administrator.');
					} else {

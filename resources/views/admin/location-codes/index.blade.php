...
@section('content')
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1 class="m-0">Location Codes</h1>
				</div>
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="#">Home</a></li>
						<li class="breadcrumb-item active">Location Codes</li>
					</ol>
				</div>
			</div>
		</div>
							...
							<table class="table table-bordered table-hover nowrap" id="dataTableBuilder">
								<thead>
									<tr>
										...
										<th>Country</th>
										<th>Country Codes</th>
										<th>Province Codes</th>
										<th>Metropolitan Codes</th>
										<th>Sub Metropolitan Codes</th>
										<th>Region</th>
										<th>ISO 3166 Alpha 2</th>
										<th>ISO 3166 Alpha 3</th>
										<th>ISO 4217 Currency Name</th>
		...
		$(document).ready( function () {
			...
			// }, 1000);
		  	...
		  	var table = $('#dataTableBuilder').DataTable({
					...
					scrollY: "400px",
					ajax: {
						url: "{{ route('location-codes.index') }}",
					  	type: 'GET',
					  	data: function (d) {
					...
					columns: [
						...
						{data:'city_province', name: 'city_province'},
						{data:'city_province_country', name: 'city_province_country'},
						{data:'country', name: 'country'},
						{data:'country_code', name: 'country_code'},
						{data:'province_code', name: 'province_code'},
						{data:'metropolitan_codes', name: 'metropolitan_codes'},
						{data:'sub_metropolitan_codes', name: 'sub_metropolitan_codes'},
						{data:'region', name: 'region'},
						{data:'iso_3166_alpha_2', name: 'iso_3166_alpha_2'},
						{data:'iso_3166_alpha_3', name: 'iso_3166_alpha_3'},
						{data:'iso_4217_currency_name', name: 'iso_4217_currency_name'},
		   ...
		   $('#delete_record').click(function(){
		      ...
		      if(delete_ids.length > 0){
		         ...
		         if (confirmdelete == true) {
		            $.ajax({
		               url: "{{ route('location-codes.multi-delete') }}",
		               type: 'POST',
